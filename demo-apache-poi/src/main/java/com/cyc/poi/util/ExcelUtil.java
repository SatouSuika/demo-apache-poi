...
public class ExcelUtil {
     ...
     * @throws IOException
     */
    public static void exportExcel(HttpServletResponse response, String headLine, String sheetName, String fileName, String[] titles, List<?> dataSet,
...
//                e.printStackTrace();
        ...
        for (int i = 0; i < columnNum; i++) {
            ...
            switch (cell.getCellTypeEnum()) {
                ...
                case NUMERIC:
                    if (DateUtil.isCellDateFormatted(cell)) {
                        DateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                        map.put(title[i], format.format(cell.getDateCellValue()));
                    } else {
                        HSSFDataFormatter dataFormatter = new HSSFDataFormatter();
                        String cellFormatted = dataFormatter.formatCellValue(cell);
                        map.put(title[i], cellFormatted);
                    }
                    break;
                case STRING:
     ...
     * @throws IOException
     */
    private static <T> void generateContent(String headLine, String[] titles, List<T> dataSet, WriteRowMapper rowMapper, Workbook workBook, Sheet sheet) throws IOException {
        //生成第一行(表头)
